...
int remainderPower2(int x, int n)
{
   ...
   int s_mask = sign << n;
   int remainder2 = !remainder + ~0;

   return (s_mask & remainder2) | remainder;
}
/* 
...
unsigned floatScale4(unsigned uf)
{
   ...
   if ((exp == 0 && frac == 0) || (exp == 0xFF && frac != 0))
      return uf;
...
int satMul2(int x)
{
   ...
   int tmin = 1 << 31;
   int x_2 = x << 1;
   int s_change = (x ^ x_2) >> 31;
   int res = s_change & (sign ^ ~tmin);
   return res | (~s_change & x_2);
}
/* 
...
{
   int sign = x >> 31;
   int x_no_sign = x & ~(0x1 << 31);
   int x_tow = ~x_no_sign + 1;

   return (sign & x_tow) | (~sign & x);
}
